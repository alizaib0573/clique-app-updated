...
const Header: React.FC<Props> = ({ goToLabel }) => {
  ...
  useGSAP(() => {
    ...
    tl.current = gsap.timeline({
      scrollTrigger: {
        ...
        start: "top top",
        end: "+=1000", // how long before nav fades out
        scrub: true,
      },
    });
  ...
  return (
    <>
      {/* Needed scroll area */}
      <div ref={containerRef} className="absolute top-0 left-0 h-[200vh] w-full -z-50" />
      ...
      >
        ...
        <div
          ref={navRef}
          className="group relative hidden h-10 w-10 items-center justify-center gap-10 rounded transition-all md:flex"
        >
          <div className="absolute top-10 right-0 flex flex-col pt-2.5">
            <div className="flex flex-col gap-1 rounded p-2">
              <button onClick={() => goToLabel("direct-to-fan")} className="cursor-pointer text-right font-mono text-lg whitespace-nowrap uppercase transition duration-300 hover:text-red-500 hover:drop-shadow-[0_0_10px_red]">
                Direct to Fan
              </button>

              <button onClick={() => goToLabel("monetization")} className="cursor-pointer text-right font-mono text-lg whitespace-nowrap uppercase transition duration-300 hover:text-red-500 hover:drop-shadow-[0_0_10px_red]">
                Your App Your Features
              </button>

              <button onClick={() => goToLabel("your-app")} className="cursor-pointer text-right font-mono text-lg whitespace-nowrap uppercase transition duration-300 hover:text-red-500 hover:drop-shadow-[0_0_10px_red]">
                Monetization
              </button>
